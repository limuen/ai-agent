...
基于 Hono 的 API 应用，通过 Wrangler 本地运行和部署到 Cloudflare Workers。当前还处于项目建设阶段，已经完成基础路由、统一响应结构、环境变量校验和 typed RPC 契约，后续会继续扩展真实业务接口。

## 职责

- 承载后续业务 API 和前端 typed RPC 调用。
- 当前提供 `GET /health` 健康检查接口。
- 当前提供 `POST /rpc/system/ping` typed RPC 基础接口。
